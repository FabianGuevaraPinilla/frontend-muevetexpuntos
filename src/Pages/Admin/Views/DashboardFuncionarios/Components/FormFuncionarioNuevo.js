...
import "./FormFuncionarioNuevo.scss"
...
export default class FormFuncionariooNuevo extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      rediret: false,
  ...
  guardarCrudSimple() {
    ...
    console.log(this.state.dataFuncionario)
    request
      .post(urlAPIFuncionarios, this.state.dataFuncionario)
      .then((response) => {
  ...
  consultarCategoria() {
    ...
    request.
        get(`${urlAPIFuncionarios}/categorias`)
        .then((response) => {
            this.setState({
